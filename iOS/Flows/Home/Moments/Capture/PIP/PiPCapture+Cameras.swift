...
extension PiPCapture {
    ...
    func configureBackCamera() -> Bool {
        ...
        do {
            ...
            logDebug("Could not create back camera device input: \(error)")
            return false
        }
        ...
        self.backOutput.setSampleBufferDelegate(self.delegate, queue: self.dataOutputQueue)
        ...
        let backCameraVideoPreviewLayerConnection = AVCaptureConnection(inputPort: backCameraVideoPort,
                                                                        videoPreviewLayer: self.backPreviewLayer)
    ...
    func configureFrontCamera() -> Bool {
        ...
        self.frontOutput.setSampleBufferDelegate(self.delegate, queue: dataOutputQueue)
        ...
        let frontCameraVideoPreviewLayerConnection = AVCaptureConnection(inputPort: frontCameraVideoPort,
                                                                         videoPreviewLayer: self.frontPreviewLayer)
        guard self.session.canAddConnection(frontCameraVideoPreviewLayerConnection) else {
            logDebug("Could not add a connection to the front camera video preview layer")
            return false
        }
