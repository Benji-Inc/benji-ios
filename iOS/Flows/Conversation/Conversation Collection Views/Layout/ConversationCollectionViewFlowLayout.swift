...
class ConversationCollectionViewFlowLayout: UICollectionViewFlowLayout {
    ...
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        guard let attributesArray = super.layoutAttributesForElements(in: rect) as? [ConversationCollectionViewLayoutAttributes] else {
            return nil
        }
    ...
    override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        guard let attributes = super.layoutAttributesForItem(at: indexPath) as? ConversationCollectionViewLayoutAttributes else {
            return nil
        }
    ...
    private func configurer(for message: Messageable?, at indexPath: IndexPath) -> ConversationCellAttributesConfigurer {
        ...
        if let kind = message?.kind {
            ...
            case .text(let text):
                if text.isEmpty {
                    return ConversationCellAttributesConfigurer()
                }
            ...
            case .link(_):
                return LinkCellAttributesConfigurer()
            }
        }
    ...
    private func headerConfigurer(for section: Int) -> ConversationHeaderAttributesConfigurer? {
        return nil
    }
